...
#define MAX_NIC_SIZE  5
#define GBPS(bytes) (bytes * 8.0 / (1000 * 1000 * 1000))

extern pthread_mutex_t mylock;
extern struct rte_ring * mtcp_recv_ring;
extern struct rte_ring * mtcp_send_ring;
...
static void GetMacAddress(int rx_inf, uint8_t ** mac){
	mac[rx_inf] = malloc(6);
	for (int i = 0; i < 6; i++){
		mac[rx_inf][i] = CONFIG.eths[rx_inf].haddr[i];
	}
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static int WriteFromVpp(struct mtcp_thread_context * ctx, int nif){
	struct dpdk_private_context * dpc;
	int write_len;
	void * temp_rte;
	dpc = (struct dpdk_private_context *)ctx->io_private_context;
	//now it can only do the output through the input eth, so it can not be used to do the forward
	uint8_t * buf;
	//printf("mTCP output\n");
        ...
        //}
        //printf("\n");
	while (!rte_ring_dequeue(mtcp_recv_ring, &temp_rte)){
		write_len = ((struct rte_mbuf *)temp_rte)->data_len;
		uint8_t * pkt = rte_pktmbuf_mtod((struct rte_mbuf *)temp_rte, uint8_t *);
		//printf("MAX_PKT_BURST = %d\n", MAX_PKT_BURST);
		if (unlikely(dpc->wmbufs[nif].len == MAX_PKT_BURST))
			return 0;
		buf = ctx->mtcp_manager->iom->get_wptr(ctx, nif, write_len);
		memcpy(buf, pkt, write_len);
		//len_of_mbuf = dpc->wmbufs[nif].len;
		//m = dpc->wmbufs[nif].m_table[len_of_mbuf];
		//memcpy(m, temp_rte, sizeof(struct rte_mbuf));
		//dpc->wmbufs[nif].len = len_of_mbuf + 1;
		//printf("dpc len = %d\n", dpc->wmbufs[nif].len);
		//uint8_t * ptr = (void *)rte_pktmbuf_mtod(m, struct uint8_t *);
		//printf("mtcp recvive packets : ");
...
/*----------------------------------------------------------------------------*/
static int is_ARP(char * pkts){
	if(pkts[12] == 8 && pkts[13] == 6){
		return 1;
	}
	else
		return 0;
...
RunMainLoop(struct mtcp_thread_context *ctx)
{
	...
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {
		...
		for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
			...
			//printf("VPP initialization finished!\n");
			for (i = 0; i < recv_cnt; i++) {
				...
				//if(pktbuf[41] == 10){
				//	start = clock();
				//	printf("start = %f", (double)start/1000);
				//}
				//for(int i = 0; i < len; i++)
				//	printf("%02x", pktbuf[i]);
				//printf("\n");
				if (pktbuf != NULL){
					if ((is_ARP((char *)pktbuf) || is_TCP((char *)pktbuf))){
						ProcessPacket(mtcp, rx_inf, ts, pktbuf, len);
					}
					if (!is_TCP((char *)pktbuf)){
						if(is_ARP((char *)pktbuf)){
							printf("receive arp packets and copy to vpp\n");
						}
						struct dpdk_private_context * dpc = (struct dpdk_private_context *)mtcp->ctx->io_private_context;
						struct rte_mbuf * m = dpc->pkts_burst[i];
						if(rte_ring_enqueue(mtcp_send_ring, m))
							printf("failed to enqueue the packets\n");
						/*
						if(n_bytes_left == 0){
							former
							memcpy(tm->iovecs[vec_count].iov_base,pktbuf,len);
							vec_count ++;
                                       			n_bytes_left = len;
				...
				//if(pktbuf[41] == 10){
				//	start = clock();
				//	printf("start = %f", (double)start);
				//}

					//WriteFromVpp(ctx, rx_inf);
				}
				else
...
mtcp_create_context(int cpu)
{
	...
	/* Wake up mTCP hreads (wake up I/O threads) */
...
/*
			...
			strcat(path," -c /etc/vpp/startup.conf");
			struct for_para * para;
                        para = malloc(100);
			para->argc = num;
			para->argv = path;
			if (pthread_create(&g_thread[cpu],NULL,vpp_main,para) != 0) {
				TRACE_ERROR("pthread_create of vpp thread failed!\n");
				return NULL;
			}
*/
	...
	extern u8 ** mac_addr;
	mac_addr = malloc(5 * sizeof(u8 *));
	for(int i = 0; i < CONFIG.eths_num; i++)
		GetMacAddress(i, mac_addr);
...
mtcp_init(const char *config_file)
{
	...
	/*
	mtcp_send_ring = rte_ring_create("mtcp_to_vpp", ring_size, rte_socket_id(), flags);
        mtcp_recv_ring = rte_ring_create("vpp_to_mtcp", ring_size, rte_socket_id(), flags);
        if (mtcp_send_ring == NULL)
                rte_exit(EXIT_FAILURE, "problem getting sending ring\n");
        if (mtcp_recv_ring == NULL)
                rte_exit(EXIT_FAILURE, "problem getting recving ring\n");
	*/
	return 0;
}
/*----------------------------------------------------------------------------*/
